...
GetOptions
 (
 "f|file=s"    => \$frq_file,   # input frequency filename (from plink)
...
my $plink = '/homes/athosnew/Genetics_Centre_Bioinformatics/resourses/plink_linux_x86_64/plink';
...
while (<IN>)
 ...
 #split line
 my @temp = split/\s+/;
 if ($temp[0] <= 22 or ($kgflag and $temp[0] == 23)) # no X, Y, XY or MT in release 1 HRC so skip checking these for now, only check if 1000G
  {
...
sub check_strand
 ...
 if ($diff > 0.2)# removed the following for A/T G/C: and ($a1 eq 'A:T' or $a1 eq 'T:A' or $a1 eq 'G:C' or $a1 eq 'C:G'))
  {
  print E "$id\n";
...
sub usage
 {
 print "Usage:\nFor HRC:\nperl HRC-1000G-check-bim-v4.2.pl -b <bim file> -f <Frequency file> -r <Reference panel> -h\n";
 print "\nFor 1000G:\nperl HRC-1000G-check-bim-v4.2.pl -b <bim file> -f <Frequency file> -r <Reference panel> -g -p <population>\n";
 }
